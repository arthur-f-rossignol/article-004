...
initialize_results <- function(n_sp,
  ...
  return(list(alpha_estimates       = rep(NA, n_sp),
...
fit_JSDM.3 <- function(Y,
  ...
  intercept_id <- seq(1, n_sp * 2, by = 2)
  x1_id        <- seq(2, n_sp * 2, by = 2)
...
results <- list(task_id      = task_id,
                ...
                k_responding = k_responding,
                seed         = data$seed,
                parameters = list(n_obs      = DEFAULT_PARAMS$n_obs,
