################################################################################
##                                                                            ##
##        JSDMs WITHOUT MISSING COVARIATE AND WITH ONE LATENT VARIABLE        ##
##                                                                            ##
...
fit_JSDM.1 <- function(Y, 
  ...
  xcoef <- fit$params$Xcoef
  xcoef_sd <- fit$sd$Xcoef
  result$beta_estimates              <- as.vector(xcoef[, 1])
  result$beta_standard_errors        <- as.vector(xcoef_sd[, 1])
  result$gamma_estimates              <- as.vector(xcoef[, 2])
  result$gamma_standard_errors        <- as.vector(xcoef_sd[, 2])
...
fit_JSDM.2 <- function(Y, 
  ...
  # Latent variables W (only if num_lv > 0)
  if (num_lv > 0 && !is.null(fit$mcmc.latent) && length(fit$mcmc.latent) > 0) {
  ...
  for (j in 1:p) {
    ...
    if (length(ess_all) >= 3) ess_gamma[j]        <- ess_all[3]
  }
...
fit_JSDM.3 <- function(Y, 
  ...
  n_fixed <- 3
  alpha_idx <- seq(1, p * n_fixed, by = n_fixed)
  beta_idx  <- seq(2, p * n_fixed, by = n_fixed)
  gamma_idx <- seq(3, p * n_fixed, by = n_fixed)
  
  result$alpha_standard_errors <- apply(beta_samples[, alpha_idx, drop = FALSE], 2, sd)
  result$beta_standard_errors  <- apply(beta_samples[, beta_idx, drop = FALSE], 2, sd)
  result$gamma_standard_errors <- apply(beta_samples[, gamma_idx, drop = FALSE], 2, sd)
  ...
  result$ess_alpha <- mean(ess_values[alpha_idx], na.rm = TRUE)
  result$ess_beta        <- mean(ess_values[beta_idx], na.rm = TRUE)
  result$ess_gamma        <- mean(ess_values[gamma_idx], na.rm = TRUE)
...
fit_JSDM.4 <- function(Y, 
  ...
  Y <- as.matrix(Y)
  X_mat <- cbind(1, as.matrix(X1), as.matrix(X2))  # Intercept + X1 + X2

  n_obs     <- nrow(Y)
  p_species <- ncol(Y)
  n_covars  <- ncol(X_mat)  # 3
  ...
  if (num_lv == 0) {
    loading_info <- list(
      indices_idx_s = matrix(0, nrow = 0, ncol = 2),
      n_idx_s       = 0,
      sum_idx_s     = 0,
  ...
  if (length(envsp_cols) > 0) {
    ...
    result$ess_gamma <- mean(sapply(1:p_species, function(sp) {
      effectiveSize(as.mcmc(envsp_arr[, 3, sp]))
    }), na.rm = TRUE)
  }
...
fit_JSDM.5 <- function(Y, 
  ...
  ess_gamma        <- sapply(1:p_species, function(sp) effectiveSize(as.mcmc(beta_arr[, sp, 2])))
  result$ess_alpha <- mean(ess_alpha, na.rm = TRUE)
  result$ess_beta  <- mean(ess_beta, na.rm = TRUE)
  result$ess_gamma <- mean(ess_gamma, na.rm = TRUE)
