...
fit_JSDM.1 <- function(Y, 
  ...
  if (!requireNamespace("gllvm", quietly = TRUE)) {
    stop("Package 'gllvm' is required for JSDM.1")
  }
  ...
  # Latent variables and loadings (only if num_lv > 0)
  if (fit$num.lv > 0) {
...
fit_JSDM.6 <- function(Y, 
  ...
  model_file <- file.path(
    tempdir(),
    sprintf("jsdm6_fixeddiag_%s_%04d.txt", random_id, sample.int(9999, 1))
  )
  writeLines(model_string, model_file)
...
run_JSDM <- function(seed, 
  ...
  data <- simulate_data(seed = seed, scenario = scenario, params = params)
